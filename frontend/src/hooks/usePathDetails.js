...
/**
 * Custom hook to manage the state and logic for the PathDetailPage component.
 * @returns {JSX.Element}
 */
const usePathDetails = () => {
  ...
  /**
   * Fetch the path locations from the backend and update the state.
  ...
  /**
   * Fetch all locations from the backend and update the state.
   * @returns {Promise<void>}
   */
  ...
  const newLocationIsValid = () => {
    ...
    if (source === "recent" && !existingLocationId) {
      console.log("existingLocationId: ", existingLocationId);
